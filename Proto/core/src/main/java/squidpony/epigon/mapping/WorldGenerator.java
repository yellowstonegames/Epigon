...
public class WorldGenerator {
    ...
    public EpiMap buildWorld(EpiMap map, HandBuilt handBuilt) {
            ...
            for (int y = 0; y < height; y++) {
                ...
                switch (c) {
                    ...
                    default:
                        ...
                        tile.floor.symbol = map.altSymbolOf(c);
                        tile.floor.color = map.colorOf(c);
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                //for (int z = 0; z < depth; z++) {
                    EpiTile tile = world.contents[x][y];
                    if (tile == null) {
                        tile = new EpiTile();
                        world.contents[x][y] = tile;
                    }
    ...
    private void bubbleMap(boolean useExistingFloor) {
        ...
        for (int growStep = 0; growStep < quantity; growStep++) {
            if (counter <= 0) {
                ...
                if (useExistingFloor) {
                    blueprint = world.contents[centerX][centerY].floor;
                } else {
            ...
            //for (int bubbleGrowZ = centerZ - bubbleSizeZ - rng.nextInt(1); bubbleGrowZ < (centerZ + bubbleSizeZ + rng.nextInt(1)); bubbleGrowZ++) {
                for (int bubbleGrowX = bubbleGrowXStart; bubbleGrowX < bubbleGrowXEnd; bubbleGrowX++) {
                    for (int bubbleGrowY = centerY - bubbleSizeY - rng.nextInt(2); bubbleGrowY < (centerY + bubbleSizeY + rng.nextInt(2)); bubbleGrowY++) {
                        if (pointInBounds(bubbleGrowX, bubbleGrowY, 0)) {
                            world.contents[bubbleGrowX][bubbleGrowY].floor = blueprint;
                        }
                        ...
                        for (int m = 0; m < rng.nextInt(3); m++) {
                            ...
                            if (pointInBounds(newX, newY, 0)) {
                                world.contents[newX][newY].floor = blueprint;
                            }
                        }
                    }
                }
