...
public class OldMap<K, V> {
    ...
    public OldMap (int initialCapacity) {
        this(initialCapacity, 0.75f);
    }

    public OldMap (int initialCapacity, float loadFactor) {
    ...
    public V put (K key, V value) {
        ...
        while (true) {
            ...
            if (existingKey == null || existingKey.equals(key)) break;
            index = index + 1 & mask;
        }
        keyTable[index] = key;
    ...
    public void putAll (OldMap<K, V> map) {
    ...
    public V get (K key) {
        ...
        while (true) {
            ...
            if (existingKey.equals(key)) return valueTable[index];
            index++;
        }
    }
    ...
    private void resize (int newSize) {
        ...
        for (int i = 0, n = keyTable.length; i < n; i++) {
            K key = keyTable[i];
            if (key != null) {
                int index = key.hashCode() & newMask;
                while (true) {
    ...
    static public class Entries<K, V> implements Iterable<Entry<K, V>>, Iterator<Entry<K, V>> {
        ...
        int index = -1;
        private final OldMap map;

        public Entries (OldMap map) {
            this.map = map;
        }
    ...
    static public class Keys<K> implements Iterable<K>, Iterator<K> {
        ...
        public Array<K> toArray () {
            Array<K> array = new Array<K>(map.size);
            while (hasNext())
                array.add(next());
            return array;
        }
